...
- **실시간 설문조사 (Poll)**: 숫자 모드(1, 2, 3...) 및 텍스트 모드(사용자 정의 선택지) 지원, 실시간 막대 그래프 결과 표시, 종료 전 투표 변경 가능
...
- 투표 변경 가능: 설문 종료 전까지 다른 선택지를 클릭하면 투표가 변경됨 (1인 1표 유지)
...
| 투표 변경 | 학생 탭에서 다른 버튼 클릭 | 이전 투표 취소 + 새 투표 반영 (1인 1표 유지) |
...
- 설문 종료 전까지 다른 선택지를 클릭하여 투표를 변경할 수 있음 (1인 1표 유지)
- 서버에서 이전 투표를 취소하고 새 투표를 반영 (브라우저, VS Code Viewer Chat 모두 지원)
