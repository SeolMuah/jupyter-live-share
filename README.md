...
**판서 좌표 정확도 향상 및 커서 표시 개선**

- **yPixel 혼합 좌표계**: 셀 높이 비율(`yRatio`) + 절대 픽셀 오프셋(`yPixel`) 병행으로 환경 간 판서 위치 정확도 대폭 향상 (하위 호환 유지)
- **코드 셀 커서 영구 표시**: 코드 셀에서 idle 타임아웃 제거 → 마지막 위치에서 영구 깜빡임
- **마크다운 셀 렌더링 모드 자동 복원**: 마크다운 셀 전용 타이머(3초)로 idle 후 렌더링 모드 복원 (코드 셀 미영향)
- **3x 뷰포트 버퍼 캔버스**: 캔버스를 뷰포트 3배 높이로 확장하여 스크롤 시 판서가 부드럽게 표시
- **CSS 폰트 정규화**: `.cell-markup`에 font-size/font-family 명시적 지정으로 VS Code webview와 웹 브라우저 간 렌더링 일관성 확보
- **HTML 구조 통일**: VS Code webview에 `#app-layout` 래퍼 추가, 웹 브라우저와 동일한 레이아웃
